...
function setPanelTrack(t){
  ...
  try{
    ...
    img.onerror=()=>{if(token!==coverToken||box.dataset.key!==key)return;box.innerHTML=noCoverUrl()?`<img class="vkl-cover-art" src="${noCoverUrl()}">`:'🎵';box.dataset.coverReady='1'};
...
function showSettingsModal(){const m=document.createElement('div');m.className='vkl-settings-modal';m.innerHTML=`<div class="vkl-settings-content"><h3>Настройки</h3><label><span>Genius Client Access Token</span><input type="password" id="vkl-genius-input" value="${geniusToken}" placeholder="Вставьте токен…"></label><div class="vkl-settings-buttons"><button id="vkl-save-genius">Сохранить</button><button id="vkl-close-settings">Закрыть</button></div></div>`;document.body.appendChild(m);m.querySelector('#vkl-save-genius').onclick=()=>{geniusToken=m.querySelector('#vkl-genius-input').value.trim();saveSettings();m.remove();if(currentTrack)fetchLyrics(currentTrack)};m.querySelector('#vkl-close-settings').onclick=()=>m.remove();m.onclick=e=>{if(e.target===m)m.remove()}}
